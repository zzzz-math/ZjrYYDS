...
Tensor_resize = transforms.Resize((224, 224))

class VAE(nn.Module):  
    def __init__(self, input_dim = 224*224, hidden_dim = 400, latent_dim = 16):  
        super(VAE, self).__init__()
        self.densenet121 = models.densenet121(pretrained=True)  
  
        # 获取最后一层之前的所有层  
        self.layers = nn.Sequential(*list(self.densenet121.children())[:-1])  
        # self.layers = layers
        # 创建一个新的全连接层，输出通道数为新的类别数  
        # new_fc = nn.Linear(1024, hidden_dim)  
        # layers.append(new_fc)
        # self.new_densenet = nn.Sequential(*layers)
        self.latent_dim = latent_dim
        self.encoder = nn.Sequential(
            nn.Flatten(),
            # self.layers,  # 全连接层，将DenseNet的输出映射到隐藏层  
            nn.Linear(50176, 1024),
            nn.ReLU(),
            nn.Linear(1024, hidden_dim),
            nn.ReLU(),  # 非线性激活函数  
            nn.Linear(hidden_dim, latent_dim * 2)  # 全连接层，将隐藏层映射到潜在空间的参数  
        )  
        self.decoder = nn.Sequential(  
            # self.densenet121,  # 使用预训练的DenseNet121模型作为backbone  
            # nn.Flatten(),  # 将DenseNet的输出展平为一维向量  
            # nn.Linear(latent_dim, hidden_dim),  # 全连接层，将潜在空间映射到隐藏层  
            # nn.ReLU(),  # 非线性激活函数
            ...
            # nn.Linear(hidden_dim, input_dim),  # 全连接层，将隐藏层映射到原始空间  
            nn.Sigmoid()  # Sigmoid激活函数，将输出限制在0和1之间  
        )  
    ...
    def forward(self, x):
        ...
        z = z.view(4,4)
        z = torch.unsqueeze(torch.unsqueeze(z,dim = 0), dim = 0)
        # print(z)
        # z = self.densenet121(z)  
...
parser.add_argument('--batch_size', type=int, default=256, metavar='N', help='batch size for training(default: 128)')
...
parser.add_argument('--num_worker', type=int, default=8, metavar='N', help='the number of workers')
...
def dataloader(batch_size=256, num_workers=8):
    ...
    return test_dataset, train_dataset, classes
...
def main():
    ...
    # Step 2: 准备工作 : 搭建计算流程
    model = VAE().to(device)  # 生成AE模型，并转移到GPU上去
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_index, (x, _) in enumerate(mnist_train):
            ...
            if (batch_index + 1) % 100 == 0:
                print('Epoch [{}/{}], Batch [{}/{}] : Total-loss = {:.4f}, BCE-Loss = {:.4f}, KLD-loss = {:.4f}, time = {}'
                      .format(epoch + 1, args.epochs, batch_index + 1, len(mnist_train) // args.batch_size,
                              loss.item() / args.batch_size, BCE.item() / args.batch_size,
                              KLD.item() / args.batch_size, execution_time))

            if batch_index == 0:
                # visualize reconstructed result at the beginning of each epoch
                x_concat = torch.cat([x.view(-1, 3, 32, 32), x_hat.view(-1, 3, 32, 32)], dim=3)
                save_image(x_concat, './%s/reconstructed-%d.png' % (args.result_dir, epoch + 1))

        # 把这一个epoch的每一个样本的平均损失存起来
        loss_epoch.append(np.sum(loss_batch) / len(mnist_train))  # len(mnist_train.dataset)为样本个数
